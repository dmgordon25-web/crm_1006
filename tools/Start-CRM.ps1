...
try {
  if ($debugging) {
    ...
    if (-not $env:CRM_SERVER_STARTED) {
      ...
      $launcherExe = $null
      try { $launcherExe = (Get-Process -Id $PID -ErrorAction Stop).Path } catch {}
      if (-not $launcherExe) {
        $exeCommand = Get-Command -Name 'powershell.exe' -ErrorAction SilentlyContinue
        if ($exeCommand) { $launcherExe = $exeCommand.Source }
      }
      if (-not $launcherExe) { $launcherExe = 'powershell.exe' }

      $script:ServerProcess = Start-Process -FilePath $launcherExe -ArgumentList $processArgs -PassThru -WorkingDirectory $PSScriptRoot -WindowStyle Hidden
      $env:CRM_SERVER_STARTED = 1
    }

    if (-not $script:ServerProcess) { throw "Failed to create CRM server process." }
    ...
    for ($i = 0; $i -lt $pollIterations; $i++) {
      ...
      if ($script:ServerProcess.HasExited) {
        ...
        throw "CRM server process exited before becoming ready (exit code $exitCode)."
      }
